          ...
          try {
            ...
            try {
              ...
              if (elements.size() > 0) {
                ...
                e->socket->stream() << pb::ircstream::reply(e, u8"%C?GREEN&B^&N " + title);
              } else {
                e->socket->stream() << pb::ircstream::reply(e, u8"%C?BLUE&B^&N <no title>");
              }
            } catch (std::exception& exc) {
              e->socket->stream() << pb::ircstream::reply(e, u8"%C?RED&B^&N <error>");
              delete root;
              xmlFreeDoc(doc);
              throw exc;
            }
